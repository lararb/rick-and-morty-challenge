...
const Header = ({ onClick, onChange, filters, onFilterByField, showFilters }) => {
    ...
    const filterTag = () => Object.entries(filters).map((filter, index) => {
        if (filter[1] !== undefined) return (
    ...
    return (
        <div data-testid={ 'Header' } className={ 'HeaderContainer' }>
            <HashRouter><Link to={'/'} className={ 'HeaderTitle' }><h1>Rick & Morty characters</h1></Link></HashRouter>
            {showFilters &&
                <>
                    <div className={ 'FiltersContainer' }>{filterTag()}</div>
                        <div className={ 'InputsContainer' }>
                            <div className={ 'CheckboxInput' }>
                                <input id={ 'inputCheckbox' } type={ 'checkbox' } onClick={event => onClick(event)}/>
