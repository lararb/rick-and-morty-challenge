...
const DetailView = ({
  ...
  const getDataCharacter = () => {
    getSingleCharacter(matchId)
      .then((data) => {
        ...
        const episodesIds =
          data.episode &&
          data.episode.map((ep) => parseInt(ep.split("/")[5], 10));
  ...
  const genderIcons =
    character && character.gender
      ? GENDER_ICONS[character.gender.toLowerCase()]
      : "";
  const statusIcons =
    character && character.status
      ? STATUS_ICONS[character.status.toLowerCase()]
      : "";

  return (
    <div className={"DetailWrapper"}>
      {isApiError && <Warning getData={getAllData} />}
        ...
        <div className={"DetailContainer"}>
          ...
          <div className={"EpisodesContainer"}>
            ...
            <ul className={"EpisodeList"}>
              {episodes &&
                finalEpisodes.map((ep, index) => (
                  <li key={`episode-${index}`} className={"EpisodeItem"}>
                    {ep.name}
                  </li>
                ))}
            </ul>
